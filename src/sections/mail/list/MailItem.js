...
import { getMails, getLabels } from '../../../store/slices/mail';
...
export default function MailItem({
  ...
  const [isImportant, setIsImportant] = useState(mail.isImportant);

  // const isUnread = !mail.isUnread;
  const [isUnread, setIsUnread] = useState(mail.isUnread);
  useEffect(() => {
    setIsUnread(mail.isUnread);
  }, [mail.isUnread])
  ...
  const handleChangeStarred = async (checked) => {
    const id = mail.id;
    ...
    await axios.put("/api/mail/mail/?mailId=" + id, raw, { headers: {'Content-Type': 'application/json'} })
    .then((response) => { setIsStarred(checked) })
    .catch((error) => { console.log(error); });
  };


  const handleChangeImportant = async (checked) => {
    const id = mail.id;
    var raw = JSON.stringify({
      "action": "important",
      "value" : checked,
    ...
    await axios.put("/api/mail/mail/?mailId=" + id, raw, { headers: {'Content-Type': 'application/json'} })
    .then((response) => { setIsImportant(checked) })
    .catch((error) => { console.log(error); });
  };
  ...
  const handleDelete = async () => {
    ...
    await axios.put("/api/mail/mail/?mailId=" + id, raw, { headers: {'Content-Type': 'application/json'} })
    .then((response) => { dispatch(getMails(params)) })
    .catch((error) => { console.log(error); });
        ...
        <Stack direction="row" alignItems="center" sx={{ mr: 2 }}>
          ...
          <Tooltip title="Starred">
            <Checkbox
              ...
              onChange={(event) => handleChangeStarred(event.target.checked)}
              checked={isStarred}
            />
          </Tooltip>
          ...
          <Tooltip title="Important">
            <Checkbox
              ...
              onChange={(event) => handleChangeImportant(event.target.checked)}
              checked={isImportant}
            />
          </Tooltip>
        </Stack>
